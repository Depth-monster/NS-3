...
main(int argc, char* argv[])
{
    ...
    Ipv4InterfaceContainer interfaces1 = address1.Assign(devices);


    UdpEchoServerHelper echoServer(17);
    UdpEchoServerHelper echoServer1(18);

    ApplicationContainer serverApps = echoServer.Install(nodes.Get(1));/*put 1*/
    ...
    UdpEchoClientHelper echoClient1(interfaces.GetAddress(1), 18);
    echoClient.SetAttribute("MaxPackets", UintegerValue(1));
    echoClient.SetAttribute("Interval", TimeValue(Seconds(1.0)));
    echoClient.SetAttribute("PacketSize", UintegerValue(1024));
